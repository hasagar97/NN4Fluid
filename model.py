...
def GeneratorBE(z, filters, output_shape, name='G',
    ...
    with tf.variable_scope(name, reuse=reuse) as vs:
        if repeat == 0:
            ...
            repeat_num = repeat
        print("Values before assert, repeat_num:",repeat_num," np.sum([i % np.power(2, repeat_num-1) for i in output_shape[:-1]])", np.sum([i % np.power(2, repeat_num-1) for i in output_shape[:-1]]))
